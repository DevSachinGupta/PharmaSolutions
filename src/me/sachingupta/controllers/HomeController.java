...
public class HomeController {
    ...
    private DB emp,sales,Product,patient;
    ...
    private void reteriveDataToObjects(){
        try {
            if (veEmployeePane.isVisible()) {
                ...
                emp=new DB("Employee",Employee.class);
                List<Employee> employees=emp.retrieveColumns("emp_id","emp_name","emp_phone","emp_email","emp_job_title","emp_department","emp_shift","emp_type","emp_address","emp_start_date","emp_end_date");
            ...
            if (vPatientPane.isVisible()) {
                ...
                patient=new DB("Patient",Patient.class);
                List<Patient> patients=patient.retrieveAll();
                for(Patient pat:patients){
                 patientTableData.add(pat);
                }
            }
            if (vProductPane.isVisible()) {
                Product=new DB("Products",Products.class);
                List<Products> products=Product.retrieveColumns("product_code","product_name","product_brand","product_category","department","type","pack_size","quantity","generic_name","expiary_date","mrp");
            ...
            if (vSalePane.isVisible()) {
                saleTableData.clear();
                sales=new DB("Sale",Sale.class);
                List<Sale> SaleList=sales.retrieveAll();
                for (Sale sale:SaleList){
                    saleTableData.add(sale);
                }
            }
        }catch(Exception ex){}
    }
